...
public class JedisLockUnderlockTask {
    ...
    public void underLockTask() {
        ...
        JedisLockUtils.underLockTask(mockOfJedis.getRedisClient(), lockName,() ->
            result1.set(true)
        );
        boolean result2 = JedisLockUtils.underLockTask(mockOfJedis.getRedisClient(), lockName,() -> true);
    ...
    public void underLock() {
        AtomicBoolean result1 = new AtomicBoolean(false);
        JedisLock jedisLock1 = new JedisLock(mockOfJedis.getRedisClient(), lockName);
        jedisLock1.underLock(() -> result1.set(true));
        JedisLock jedisLock2 = new JedisLock(mockOfJedis.getRedisClient(), lockName);
    ...
    public void underLockWithInterrupted() {
        AtomicBoolean result1 = new AtomicBoolean(false);
        InterruptingJedisJedisLockBase jedisLock1 = new InterruptingJedisJedisLockBase(mockOfJedis.getRedisClient(), lockName, 1, TimeUnit.SECONDS);
        jedisLock1.underLock(() -> result1.set(true) );
        InterruptingJedisJedisLockBase jedisLock2 = new InterruptingJedisJedisLockBase(mockOfJedis.getRedisClient(), lockName, 1, TimeUnit.SECONDS);
        boolean result2 = jedisLock2.underLock(() -> true);
        assertTrue(result1.get());
        assertTrue(result2);
    }
