...
public class FunctionalSscanIterableTest {
    ...
    void createABCData() {
        letters.forEach( letter -> {
            redisClient.sadd(sscanitName, letter);
        });
    }
    ...
    public void iteratorEmptyTest() {
        int num = 0;
        SScanIterable sscanIterable = new SScanIterable(redisClient,sscanitName,  "*");
    ...
    public void iteratorEmpty2Test() {
        SScanIterable sscanIterable = new SScanIterable(redisClient, sscanitName, 20);
    ...
    public void iteratorWithResultKeysTest() {
        ...
        Iterator<String> iterator =  sscanIterable.iterator();
        while(iterator.hasNext()) {
            assertTrue( redisClient.exists(sscanitName));
            assertTrue( redisClient.sismember(sscanitName, iterator.next()));
        }
    }
    ...
    public void iteratorWithResultKeysForEachTest() {
        ...
        sscanIterable.forEach( key -> {
            assertTrue( redisClient.exists(sscanitName));
            assertTrue( redisClient.sismember(sscanitName, key));
        });
    }
    ...
    public void iteratorRemoveForEach1Test() {
        createABCData();
        redisClient.sadd(sscanitName, "extra");
        ...
        deleted.forEach( key -> {
            assertFalse( redisClient.sismember(sscanitName, key));
        });
    }
